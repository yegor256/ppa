...
Lexical analyzer (\hs{lexer} or \hs{scanner}) takes input language and produces \hs{tokens} (which then can be parsed into parse tree by a parser).

\hs{Lexemes} are said to be a sequence of characters in a token.
\plush{}
...
Some lexems (like spaces or EOLs) are ignored and do not become tokens. They are \hs{non-token} elements. However, they could become tokens, like in this DFA:\\
\begin{tikzpicture}[graph]
\node[double] (s) {$S$};
...
While \hs{lemmatization} focuses purely on feature extraction and data cleaning, \hs{syntactic analysis} analyzes the relationship between words and the grammatical structure of sentences.
\plush{}

\pptSection[LL/LR]{Top-Down and Bottom-Up Parsing}

\hs{Top-down parsing} builds the parse tree from the top (start
symbol) down; most top-down methods are LL.
\hs{Bottom-up parsing} builds the parse tree from the leaves
...
A \hs{recursive descent parser} is the one the checks every rule before making a decision which one is right.

\hs{Predictive parsing} is possible only for the class of \(LL(k)\) grammars, which are the CFGs for which there exists some positive integer \(k\) that allows a parser to decide which production rule to use by examining only the next \(k\) tokens of input.
\plush{}

\pptSection[CC]{Flex and Bison}

These tools are called \hs{compiler-compilers} (originally \texttt{lex} and \texttt{yacc}) or parser generators.
...
Each time the program needs a token, it calls \texttt{yylex()}, which reads a little input and returns the token. When it needs another token, it calls \texttt{yylex()} again. The scanner acts as a \hs{coroutine}; that is, each time it returns, it remembers where it was, and on the next call it picks up where it left off.
\plush{}

The \hs{action code} is what stays in the brackets after the \hs{pattern}. If action code returns, scanning resumes on the next call to \texttt{yylex()}; if it doesn't return, scanning resumes immediately.
\plush{}

Lexical errors may be \hs{handled} and the lexer may \hs{recover} from some of them: we don't want the lexer to stop at the first error. See how Flex recovers.
\plush{}
