...
The \hs{concrete syntax} of a programming language is defined by a context free grammar (CFG).
The \hs{abstract syntax} of an implementation is the set of trees used to represent programs in the implementation.
...
Somehow we must \hs{link} different $x$ to different places, where they are \hs{declared},
maybe with the help of "\hs{Identification Table}," or by attaching attributes to AST nodes, or both.
We may want to track their \hs{indentation levels}.
\plush{}

\plush{\pptChapter{Static Type Checking}}

Dynamically-typed languages perform \hs{type checking} at \hs{runtime},
while statically typed languages perform type checking at \hs{compile time}.
