...
A path is \hs{feasible} if there exists an input $\mathcal{I}$ to the program that covers the path; i.e., when program is executed with $\mathcal{I}$ as input, the path is taken.
...
A path is \hs{infeasible} if there exists no input $\mathcal{I}$ that covers the path.
...
\hs{Path condition} is a condition on the input symbols such that if a path is feasible its path-condition is satisfiable.
...
A \hs{constraint solver} is a tool that finds satisfying assignments for a \hs{constraint}, if it is satisfiable.

A \hs{solution} of the constraint is a set of assignments, one for each free variable that makes the constraint satisfiable.
...
\hs{SAT solver} is a computer program which aims to solve the \hs{Boolean satisfiability problem}: whether the variables of a given Boolean formula can be consistently replaced by the values TRUE or FALSE in such a way that the formula evaluates to TRUE.
...
\hs{SMT solver} is a computer program which aims to solve the \hs{satisfiability modulo theories}: determine whether a mathematical formula is satisfiable.
...
Symbolic execution cannot handle \hs{unsolvable} or almost unsolvable constraints.
...
\hs{Path explosion} refers to the fact that the number of control-flow paths in a program grows exponentially with an increase in program size and can even be infinite in the case of programs with unbounded loop iterations.
