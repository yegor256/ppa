...
\hs{Semantic} property can be completely defined with respect to the set of executions of a program,
while a \hs{syntactic} property can be decided directly based on the program text.
...
\hs{Rice's theorem} states that all non-trivial semantic properties of programs are undecidable.

A property is \hs{non-trivial} if it is neither true for every partial computable function, nor false for every partial computable function.

\hs{Halting problem} is the problem of determining, from 1)~a description of an arbitrary computer program and 2)~an input, whether the program will finish running, or continue to run forever. A general algorithm to solve the halting problem for all possible program–input pairs \textbf{cannot exist}.
...
Consider two C++ programs given to a \hs{static analyzer} (e.g. \href{https://clang.llvm.org/extra/clang-tidy/}{Clang Tidy}):
...
Consider two C++ programs given to a \hs{style checker} (e.g. \href{https://github.com/cpplint/cpplint}{cpplint}):
...
Consider this C++ program (Clang Tidy finds no issues) given to a \hs{dynamic analyzer}
...
``It is very easy to obtain soundness if we forsake completeness: \hs{reject} every case.''
...
\hs{Total order} is a binary relation \(\leq\) (\hs{strict total order} is \(<\)).

\hs{Linearly ordered set} (loset) is a set equipped with a total order.
...
\hs{Partial order} is total order but only between some elements.

\hs{Partially ordered set} (poset) is a set equipped with a partial order.
...
\hs{Lattice} is a poset where each two elements \((x, y)\) have \hs{least upper bound} (\hs{join} operator \(x \sqcup y\)) and
\hs{greatest lower bound} (\hs{meet} operator \(x \sqcap y\)).
...
A lattice may be used to represent \hs{intervals} in a set of values, e.g. in \(\mathbb{Z}\):
...
\begin{pptWide}{3}
\hs{Concrete domain} \(C\) \\ \((\mathbb{Z}, <)\)\par
\begin{tikzpicture}[line width=1pt,->]
\node (a) {\(12\)};
...
\end{tikzpicture}\par
\par\columnbreak\par
\hs{Abstract domain} \(A\) \\ \((\{s \vert \{ n \vert n \in \mathbb{N} \} \}, \in)\)\par
\begin{tikzpicture}[line width=1pt,->]
\node (top) {\(\mathbb{N}\)};
...
Domains must be related through \hs{Galois connection}:
...
\hs{Fixed-Point Computation} is a repeated symbolic execution of the program using abstract semantics
until approximation reaches an \hs{equilibrium}.
