...
A \hs{proof system} is formed from a set of \hs{inference rules} chained together to form proofs, also called \hs{derivations}. Any derivation has only one final conclusion, which is the statement proved or derived. (Wiki)
...
Premises (known \hs{facts}): $a < b$ and $b < c$. \\
{\small (antecedent)}
...
An \hs{axiom} is an inference rule without a premise.
...
A \hs{transition rule} defines the conditions under which a system may be moved to a new \hs{state}.
\begin{prooftree}
...
The \hs{denotational semantics} assign to every expression the \hs{number} denoted by that expression:
\begin{gather*}
...
The \hs{operational semantics} describe the computation steps taken in order to evaluate the expression to \hs{normal form}:
\begin{gather*}
...
The operational semantics is the specification of an \hs{interpreter} for the programming language whereas the denotational semantics tries to capture the \hs{mathematical essence} of a program, abstracting away from computational details.
...
It reads: If \(e\) may be \hs{evaluated} to \(n\),
...
Consider a \hs{$\lambda$-expression}:
\begin{equation*}
(\lambda a.a) b
\end{equation*}
...
The expression may be reduced using so called \hs{$\beta$-reduction}:
\begin{equation*}
...
A \hs{normal form} is a form that has no more possible applications of reductions.
...
\hs{Fundamental Concepts in Programming Languages}
