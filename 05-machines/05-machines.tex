...
An \hs{abstract machine} is a theoretical \hs{model} of computation.

Similar to a function, a machine receives \hs{inputs} and produces \hs{outputs} based on predefined \hs{rules}.

Abstract machines are ``machines'' because they allow \hs{step-by-step} execution of programmes. (really?)

They are ``abstract'' because they ignore many aspects of actual (hardware) machines.

An abstract machine is an \hs{intermediate language} with a small-step operational semantics.
...
We are interested in using abstract machines to explain the \hs{semantic} of a program.

\plush{}

\pptSection{Virtual Machines}

An abstract machine implemented in software is termed a \hs{virtual machine},
...
IR (\hs{intermediate representation}) is used internally by a compiler or virtual machine to represent source code.
An \hs{intermediate language} is the language of an abstract machine.
...
The \hs{Church-Turing thesis}: Anything that can be computed can be computed by some Turing machine.
...
In lambda calculus, \hs{functions} are taken to be ``first class values,''
