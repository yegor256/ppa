...
Data flow analysis \hs{propagates} information (\hs{data}) along the control flow graph, with the following six properties in mind:

\begin{enumerate}
    \item Domain (of \hs{data flow facts})
    \item Direction (forward or backward)
    \item \hs{Transfer Function} (sometimes with GEN and KILL sets)
    \item \hs{Confluence Operator} ("meet" or "join")
...
The \hs{meet operator} is coming from the lattice that abstracts the data that flows (remember \hs{abstract interpretation}?):
...
From the perspective of \hs{path insensitive} data flow analysis, there are bugs in both CFGs, but it's wrong:
...
A \hs{path-sensitive} analysis computes different pieces of analysis information dependent on the \hs{predicates} at conditional branch instructions.
...
A \hs{flow-sensitive} analysis takes into account the order of statements in a program.

The analysis we did before was flow sensitive. Flow \hs{insensitive} analysis example:
...
A \hs{context-sensitive} analysis is an interprocedural analysis that considers the calling context when analyzing the target of a function call.
...
\hs{Reaching definitions} is a data-flow analysis which statically determines which definitions may reach a given point in the code.
...
\hs{Live variable analysis} calculates the variables that are live at each point in the program (they hold values that may be needed in the future).
...
\hs{Definite assignment analysis} conservatively ensures that a variable or location is always assigned before it is used.
...
\hs{Available expression analysis} determines for each point in the program the set of expressions that need not be recomputed.
...
\hs{Constant propagation analysis} at every statement tells which variables is a constant:
